...
// CODE HERE
const totalPopulation = populations.reduce((accu, cur) => accu + cur)
...
const afterTax = orders.map(price =>price.price + (price.price * price.tax))
...
// CODE HERE
const bob = purchases.filter(purchase => purchase.owner === "Bob").map(bob => bob.price).reduce((acc, cur) => acc + cur )
console.log(bob)